		...
		<el-dialog :title="dialog.title" v-model="showDialog" width="540px" @close="closeDialog">
			<el-form ref="roleDialogFormRef" :model="formData" size="default" label-width="120px">
				<el-form-item label="医生名称" prop="name">
					<el-input v-model="formData.name" placeholder="请输入医生姓名" clearable></el-input>
				</el-form-item>
				<el-form-item label="医生简介" prop="name">
					<el-input type="textarea" v-model="formData.info" placeholder="项目简介" clearable></el-input>
				</el-form-item>
				<el-form-item label="性别" prop="info">
					<el-radio-group v-model="formData.gender" clearable>
						<el-radio label="男" size="large">男</el-radio>
						<el-radio label="女" size="large">女</el-radio>
					</el-radio-group>
				</el-form-item>
        <el-form-item label="生日" prop="birthday">
          <el-date-picker v-model="formData.birthday" type="date" clearable>
          </el-date-picker>
        </el-form-item>
...
const openDialog = (type, row) => {
	if (type === 'edit') {
		...
		dialog.value.title = '修改医生信息';
		dialog.value.submitTxt = '修 改';
		dialog.value.type = type
	} else {
		dialog.value.title = '新增医生信息';
